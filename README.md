...
- 💳 Продаёт Pro-доступ через СБП за 349 ₽/мес (Bot Payments 2.0)
...
6. Запустите автопланер — он обрабатывает очередь BullMQ и подбирает «зелёные окна» после выбора опций в плане. Убедитесь, что заданы `BOT_DATABASE_URL`, `REDIS_URL`, `BOT_TOKEN_DEV` и координаты по умолчанию (`WEATHER_LAT`/`WEATHER_LON`):
