...
> ℹ️ **MVP-режим:** по умолчанию бот не обращается к каталогу и показывает препараты из ответа GPT. Включите использование `product_rules`, установив `PLAN_USE_PRODUCT_RULES=1` в `.env` и перезапустив `bot`.
