...
- [ ] Firewall/сетевые политики хоста: сейчас `ufw` не активен. Рекомендуется включить и явно открыть только нужные входящие порты (обычно 22, 80, 443) и закрыть остальное.
